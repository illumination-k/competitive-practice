...
fn run<R: BufRead>(mut source: AutoSource<R>) -> impl AtCoderFormat {
    ...
    debug!(g);
    // dp[k][a] k日目に都市aにいるときの組み合わせ
    let mut dp = vec![vec![mb.build(0); n + 1]; k + 10];
    dp[0][0] = mb.build(1);

    for i in 0..k {
        // uvがなければ、遷移はただの足し算
        let sum = dp[i].iter().sum::<ModInt<usize>>();
        // ここから行けない部分を引く
        for j in 0..n {
            let minus = dp[i][j] + g[j].iter().map(|&p| dp[i][p]).sum::<ModInt<usize>>();
            dp[i + 1][j] += sum - minus;
            // let nxt = format!("dp[{}][{}] = {}", i+1, j, dp[i+1][j].value());
            // debug!(i, sum.value(), minus.value(), nxt);
        }
    }
