...
fn solve() -> impl AtCoderFormat {
    ...
    1からのそれぞれの場所への最小値
    Nからそれぞれの場所への最小値
    をダイクトストラで前計算すれば間に合いそう
    */

    let lg = ListGraph::weighted_from(abc, n, 1, Direction::UnGraph);
    let (from1, _) = diktstra(&lg, 0);
    let (fromn, _) = diktstra(&lg, n - 1);
    debug!(from1, fromn);

    let mut ans = vec![];

    for i in 0..n {
        ans.push(from1[i] + fromn[i]);
    }
