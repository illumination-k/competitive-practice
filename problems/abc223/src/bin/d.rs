...
fn tsort(g: &ListGraph<usize>) -> Vec<usize> {
    // 次数を数える
    let n = g.len();
    let mut dim = vec![0; n];
    for i in 0..n {
        for &e in g.neighbors_unweighted(i) {
            dim[e] += 1;
        }
    }
    debug!(dim);
    ...
    while let Some(Reverse(now)) = que.pop() {
        ...
        for e in g.neighbors(now) {
            dim[e.target()] -= 1;
            if dim[e.target()] == 0 {
                que.push(Reverse(e.target()))
            }
        }
    }
...
fn run() -> impl AtCoderFormat {
    ...
    let ab = ab.into_iter().unique().collect_vec();
    let g = ListGraph::unweighted_from(&ab, n, 1, Direction::DiGraph);
    debug!(g);
    let tans = tsort(&g);
    if tans.len() != n {
        return vec![-1];
    ...
    let ans = tans.iter().cloned().map(|u| u as isize + 1).collect_vec();
