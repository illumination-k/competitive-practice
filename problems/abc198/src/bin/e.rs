...
fn dfs(
    ...
    for &t in g.neighbors_unweighted(s) {
        if seen[t] {
            continue;
        }

        // seen[t] = seen[s].clone();
        dfs(g, seen, good_number, colors, c, t, Some(s));
    }
...
fn run() -> impl AtCoderFormat {
    ...
    let g = ListGraph::unweighted_from(&ab, n, 1, Direction::UnGraph);
    let mut seen = vec![false; n];
    let mut good_number = vec![false; n];
    let mut colors = vec![0; cmax + 1];

    dfs(&g, &mut seen, &mut good_number, &mut colors, &c, 0, None);
    debug!(good_number);
