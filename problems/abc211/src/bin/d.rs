...
fn solve() -> impl AtCoderFormat {
    ...
    let g: ListGraph<usize> = ListGraph::unweighted_from(&ab, n, 1, Direction::UnGraph);
...
fn bfs_with_cnt(g: &ListGraph<usize>, s: usize) -> Vec<usize> {
    let mut dist = vec![-1; g.len()];
    let mut dq = VecDeque::new();
    let mut count = vec![0; g.len()];

    dist[s] = 0;
    count[s] = 1;
    dq.push_back(s);

    while let Some(source) = dq.pop_front() {
        for &target in g.neighbors_unweighted(source) {
            if dist[target] == -1 {
                dist[target] = dist[source] + 1;
                dq.push_back(target);
                count[target] = count[source];
            } else if dist[target] == dist[source] + 1 {
                count[target] += count[source];
                count[target] %= MOD;
            }
        }
