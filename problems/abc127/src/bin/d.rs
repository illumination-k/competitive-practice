...
fn solve() -> impl AtCoderFormat {
    ...
    let mut bq: BinaryHeap<_> = a.into_iter().map(|x| std::cmp::Reverse(x)).collect();
    let mut ans = 0;
    ...
    for &(b, c) in bc.iter() {
        ...
        for _ in 0..b {
            if let Some(std::cmp::Reverse(val)) = bq.pop() {
                if val > c {
                    ans += val;
                } else {
                    ans += c;
    ...
    ans += bq.into_iter().map(|std::cmp::Reverse(x)| x).sum::<usize>();
    ans
}
